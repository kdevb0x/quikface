...
// maxCapacity is the maximum number of clients allowed to a room.
const maxCapacity = 2
...
// Client returns the client if present, otherwise if maxCapacity is not met,
// new Client is created inside the r, ( then returned.
func (r *Room) Client(clientID uint32) (*Client, error) {
	if c, exists := r.ClientList[clientID]; exists {
		return c, nil
	}
	if len(r.ClientList) >= maxCapacity {
...
func (r *Room) InitMediaSession(c1, c2 *Client) error {
	if _, found := r.ClientList[c1.ID]; !found {
		r.ClientList[c1.ID] = c1
		// return fmt.Errorf("client id not found in %s's client list. Client must be in the rooms ClientList to participate", r.Name)
	}

	if _, found := r.ClientList[c2.ID]; !found {
		r.ClientList[c2.ID] = c2
		// return fmt.Errorf("client id not found in %s client list. Client must be in the rooms ClientList to participate", r.Name)
	}
